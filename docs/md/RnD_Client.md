...
│   └── 검색 알고리즘 최적화
│
🔍 3단계: AI 에이전트 통합 및 답변 생성
    ├── IDE AI 에이전트와 검색 시스템 연동
    ├── 컨텍스트 기반 답변 생성
    └── 사용자 인터페이스 구축
```
...
🔄 연동 방식
• Chat API를 통한 컨텍스트 전달
• 코드 주석을 통한 프롬프트 엔지니어링
• Extension 개발로 검색 결과 자동 전달
• Inline completion에 검색 결과 활용
```
...
가장 관련성 높은 JSDoc이 포함된 코드 청크 검색
                    ↓
🤖 AI 에이전트 전달:
검색된 코드와 문서를 IDE AI 에이전트에게 컨텍스트로 제공
                    ↓
✨ 정확한 답변 생성:
